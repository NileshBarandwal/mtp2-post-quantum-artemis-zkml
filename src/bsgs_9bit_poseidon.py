"""
bsgs_9bit_poseidon.py — 9-bit Curve BSGS Attack + FRI+Poseidon
...
PIPELINE:
  ...
  2. Build SRS: [G, τG, τ²G, …, τᴰG]  using same 9-bit tau as BSGS target
...
OUTPUT_FILE = os.path.normpath(
    os.path.join(_SCRIPT_DIR, "..", "results", "multicurve", "bsgs_9bit_poseidon.txt")
)
...
def main():
    ...
    try:
        print()
        print("=" * 68)
        print("  KZG FORGERY VIA 9-BIT BSGS — Full End-to-End Demo")
        ...
        print("    2. Build SRS = [G, τG, τ²G, …] with random 9-bit τ")
        ...
        print(f"PC.Setup     : ✓ tau={tau} (9-bit), SRS[1]={srs[1]}")
        ...
        print("  Classical BSGS on 9-bit : instant (microseconds)")
        ...
        print("═══════════════════════════════════════════════════════")
        print("  SECURITY COMPARISON: KZG (9-bit) vs FRI+Poseidon")
        print("  Definition 2.2 — Lycklama et al. arXiv:2409.12055")
        print("═══════════════════════════════════════════════════════")
        print("  Operation      | KZG (9-bit)            | FRI+Poseidon")
        print("  ───────────────┼────────────────────────┼─────────────────────")
        print("  PC.Setup       | SRS with 9-bit tau     | nonce only, no tau")
        print("  PC.Commit      | ECC point (9-bit)      | Merkle root (32B)")
        print("  PC.Verify      | 1 ✓                    | 1 ✓")
        ...
        print("  ───────────────┼────────────────────────┼─────────────────────")
        print("  BSGS attack    | tau recovered instantly | FAILED (no group)")
