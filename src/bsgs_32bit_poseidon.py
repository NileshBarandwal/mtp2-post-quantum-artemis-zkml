"""
bsgs_32bit_poseidon.py — 32-bit Curve BSGS Attack + FRI+Poseidon
...
PIPELINE:
  ...
  2. Build SRS: [G, τG, τ²G, …, τᴰG]  using same 32-bit tau as BSGS target
...
NOTE: Uses lightweight dict-based BSGS from bsgs_attack.py, sized to sqrt(n).
      No large RAM allocation required for small curves.
"""
...
from ecc_utils_32bit import EllipticCurve, CURVE_A, CURVE_B, CURVE_P
CURVE_N = None  # will be computed at runtime
_GX = None
_GY = None
...
OUTPUT_FILE = os.path.normpath(
    os.path.join(_SCRIPT_DIR, "..", "results", "multicurve", "bsgs_32bit_poseidon.txt")
)
...
def main():
    ...
    try:
        print()
        print("=" * 68)
        print("  KZG FORGERY VIA 32-BIT BSGS — Full End-to-End Demo")
        ...
        print("    2. Build SRS = [G, τG, τ²G, …] with random 32-bit τ")
        ...
        print(f"  G      :  {G}")
        print(f"  n      :  {n}  (2^{math.log2(n):.2f})  ← group order")
        print()
        ...
        print("  " + "=" * 64)
        print("  Opening polynomial at 3 evaluation points...")
        ...
        print(f"PC.Setup     : ✓ tau={tau} (32-bit), SRS[1]={srs[1]}")
        ...
        tau_rec, baby_t, giant_t, steps = bsgs(Q, G, n, curve)
        ...
        print("═══════════════════════════════════════════════════════")
        print("  SECURITY COMPARISON: KZG (32-bit) vs FRI+Poseidon")
        print("  Definition 2.2 — Lycklama et al. arXiv:2409.12055")
        print("═══════════════════════════════════════════════════════")
        print("  Operation      | KZG (32-bit)           | FRI+Poseidon")
        print("  ───────────────┼────────────────────────┼─────────────────────")
        print("  PC.Setup       | SRS with 32-bit tau    | nonce only, no tau")
        print("  PC.Commit      | ECC point (32-bit)     | Merkle root (32B)")
        print("  PC.Verify      | 1 ✓                    | 1 ✓")
        ...
        print("  ───────────────┼────────────────────────┼─────────────────────")
        print("  BSGS attack    | tau recovered ~100ms   | FAILED (no group)")
